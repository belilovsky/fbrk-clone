...
WHERE
  -- Only overwrite if the new parse is at least as complete and something
  -- actually changed. This keeps RSS polling idempotent: identical articles no
  -- longer churn updated_at every ten minutes.
  COALESCE(length(excluded.sections_json), 0) >= COALESCE(length(articles.sections_json), 0)
  AND (
    excluded.title <> articles.title
    OR length(excluded.dek) > length(articles.dek)
    OR excluded.date_iso <> articles.date_iso
    OR excluded.date_label <> articles.date_label
    OR excluded.category <> articles.category
    OR excluded.category_label <> articles.category_label
    OR (excluded.image <> '' AND excluded.image <> articles.image)
    OR excluded.source <> articles.source
    OR excluded.body_json <> articles.body_json
    OR excluded.sections_json <> articles.sections_json
  )
"""
